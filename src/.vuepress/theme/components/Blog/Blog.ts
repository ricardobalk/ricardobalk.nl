...
export const Blog = defineComponent({
  ...
  setup() {
    ...
    const fetchPosts = (params: Record<string, any>) => {
      ...
      filterPagesByFrontmatter(params).then((pages) => pages.filter((page) => posts.push(page)));
      return posts;
    };
    ...
    const availableCategories = computed(() => {
      ...
      posts.value.forEach((post) => {
        typeof post.frontmatter.category === "string" && categories.add(post.frontmatter.category);
      });
